...
const webpayPlusTransaction = new WebpayPlus.Transaction(configuration);
...
const TBController = {
  CreateTransaction: async (req, res) => {
    ...
    try {
      const createResponse = await webpayPlusTransaction.create(
        buyOrder,
        sessionId,
        amount,
        returnUrl
  ...
  ConfirmTransaction: async (req, res) => {
    ...
    try {
      const commitResponse = await webpayPlusTransaction.commit(token);
